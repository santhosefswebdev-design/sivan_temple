...
class Archanai_booking extends BaseController
{
    ...
    public function save(){
		...
		// $pay_method_check = (!empty($_POST['pay_method']) ? $_POST['pay_method'] : 'cash');
		// if ($pay_method_check == 'cash') {
		// 	$paid_amt_check = !empty($_POST['paid_amount']) ? (float) $_POST['paid_amount'] : 0;
		// 	$total_amt_check = !empty($_POST['tot_amt']) ? (float) $_POST['tot_amt'] : 0;
		// 	if($total_amt_check > $paid_amt_check){
		...
		$data['payment_status'] = (($pay_method == 'cash' || $pay_method == 'online' || $pay_method == 'qr' || $pay_method == 'card') ? 2 : 1);
