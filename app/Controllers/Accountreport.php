...
class Accountreport extends BaseController
{
	...
	public function excel_multiple_ledger_statement()
	{
		...
		foreach($ledger_ids as $ledgerid){
			...
			$csv_arr[] = array('','','','','','','','');
			$csv_arr[] = array('Date','Ref No','Ledger','Particulars','Debit','Credit','Net Activity','Balance');
			$op_bal_txt = '';
	...
	public function excel_multiple_ledger_statement1()
	{
		...
		foreach($ledger_ids as $ledgerid){
			...
			$sheet->getStyle("A4:H4")->applyFromArray($style);
			...
			$sheet->setCellValue('C3', 'Ledger');
			$sheet->setCellValue('D3', 'Particulars');
			$sheet->setCellValue('E3', 'Debit');
			$sheet->setCellValue('F3', 'Credit');
			$sheet->setCellValue('G3', 'Net Activity');
			...
			$cu_debit = 0;
			 foreach($res_ledger['data'] as $row) { 
					...
					$sheet->setCellValue('C' . $rows, $row['ledger']);
					$sheet->setCellValue('D' . $rows, $row['narration']);
					$sheet->setCellValue('E' . $rows, $row['debit']);
					$sheet->setCellValue('F' . $rows, $row['credit']);
					$sheet->setCellValue('G' . $rows, '');
					$sheet->setCellValue('H' . $rows, $trans_amount_txt);
					$rows++;
			}
			$cl_bal_txt = '';
			if($res_ledger['cl_bal'] < 0){
			...
			$sheet->setCellValue('E' . $rows, number_format($cu_debit,'2','.',','));
			$sheet->setCellValue('F' . $rows, number_format(abs($cu_credit),'2','.',','));
			$sheet->setCellValue('G' . $rows, number_format(abs($diffrence_amt),'2','.',','));
			$sheet->setCellValue('H' . $rows, $cl_bal_txt);
			$i++;
		}
	...
	public function profile_loss()
	{
		...
		if ($breakdown == "monthly") {
			...
			$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_0 level_groups">' . $top_sales_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
			$ledger_total = array();
				...
				foreach ($sales_sub_groups as $sales_sub_group) {
					...
					if (count($sales_sub_ledgers) > 0) {
						...
						if (count($sub_table) > 0) {
							$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_1 level_groups">' . $sales_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
							...
							foreach ($sales_sub_sub_groups as $sales_sub_sub_group) {
								...
								if (count($sales_sub_sub_ledgers) > 0) {
									...
									if (count($sub_sub_table) > 0) {
										$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_2 level_groups">' . $sales_sub_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
						...
						if (count($sub_table) > 0) {
							...
							if (count($getMonthsInRange) > 0) {
								$month_total = 0;
								foreach ($getMonthsInRange as $res_month) {
									if ($sub_ledger_total[$res_month['date']] < 0) $sub_ledger_monthly_total = "( " . number_format(abs($sub_ledger_total[$res_month['date']]), 2) . " )";
									else $sub_ledger_monthly_total = number_format(abs($sub_ledger_total[$res_month['date']]), 2);
									$html .= '<td align="right" class="bold_text">' . $sub_ledger_monthly_total . '</td>';
			...
			if (count($getMonthsInRange) > 0) {
				$month_total = 0;
				foreach ($getMonthsInRange as $res_month) {
					if ($ledger_total[$res_month['date']] < 0) $ledger_monthly_total = "( " . number_format(abs($ledger_total[$res_month['date']]), 2) . " )";
					else $ledger_monthly_total = number_format(abs($ledger_total[$res_month['date']]), 2);
					$html .= '<td align="right" class="bold_text">' . $ledger_monthly_total . '</td>';
			...
			$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_0 level_groups">' . $top_co_sales_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
			$co_sales_total = 0;
			$ledger_total = array();
			...
			if (count($co_sales_sub_groups) > 0) {
				foreach ($co_sales_sub_groups as $co_sales_sub_group) {
					...
					if (count($co_sales_sub_ledgers) > 0) {
						...
						if (count($sub_table) > 0) {
							$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_1 level_groups">' . $co_sales_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
							foreach ($sub_table as $sb) {
								$table[] = $sb;
							}
						}
						...
						if (count($co_sales_sub_sub_groups) > 0) {
							foreach ($co_sales_sub_sub_groups as $co_sales_sub_sub_group) {
								...
								if (count($co_sales_sub_sub_ledgers) > 0) {
									...
									if (count($sub_sub_table) > 0) {
										$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_2 level_groups">' . $co_sales_sub_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
										foreach ($sub_sub_table as $sb) {
											$table[] = $sb;
										}
			...
			$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_0 level_groups">' . $top_incomes_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
			$incomes_total = 0;
			$ledger_total = array();
			...
			if (count($incomes_sub_groups) > 0) {
				foreach ($incomes_sub_groups as $incomes_sub_group) {
					...
					if (count($incomes_sub_ledgers) > 0) {
						...
						if (count($sub_table) > 0) {
							$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_1 level_groups">' . $incomes_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
							foreach ($sub_table as $sb) {
								$table[] = $sb;
							}
						}
						...
						if (count($incomes_sub_sub_groups) > 0) {
							foreach ($incomes_sub_sub_groups as $incomes_sub_sub_group) {
								...
								if (count($incomes_sub_sub_ledgers) > 0) {
									...
									if (count($sub_sub_table) > 0) {
										$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_2 level_groups">' . $incomes_sub_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
										foreach ($sub_sub_table as $sb) {
											$table[] = $sb;
										}
			...
			$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_0 level_groups">' . $top_expenes_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
			$ledger_total = array();
			...
			if (count($expenes_sub_groups) > 0) {
				foreach ($expenes_sub_groups as $expenes_sub_group) {
					...
					if (count($expenes_sub_ledgers) > 0) {
						...
						if (count($sub_table) > 0) {
							$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_1 level_groups">' . $expenes_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
							foreach ($sub_table as $sb) {
								$table[] = $sb;
							}
						}
						...
						if (count($expenes_sub_sub_groups) > 0) {
							foreach ($expenes_sub_sub_groups as $expenes_sub_sub_group) {
								...
								if (count($expenes_sub_sub_ledgers) > 0) {
									...
									if (count($sub_sub_table) > 0) {
										$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_2 level_groups">' . $expenes_sub_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
										foreach ($sub_sub_table as $sb) {
											$table[] = $sb;
										}
			...
			$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_0 level_groups">' . $top_taxes_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
			$ledger_total = array();
			...
			if (count($taxes_sub_groups) > 0) {
				foreach ($taxes_sub_groups as $taxes_sub_group) {
					...
					if (count($taxes_sub_ledgers) > 0) {
						...
						if (count($sub_table) > 0) {
							$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_1 level_groups">' . $taxes_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
							foreach ($sub_table as $sb) {
								$table[] = $sb;
							}
						}
						...
						if (count($taxes_sub_sub_groups) > 0) {
							foreach ($taxes_sub_sub_groups as $taxes_sub_sub_group) {
								...
								if (count($taxes_sub_sub_ledgers) > 0) {
									...
									if (count($sub_sub_table) > 0) {
										$table[] = '<tr><td style="font-weight: bold;font-size: medium;" class="level_2 level_groups">' . $taxes_sub_sub_group['name'] . '</td><td colspan="' . ( count($getMonthsInRange) + 1)  . '"></td></tr>';
										foreach ($sub_sub_table as $sb) {
											$table[] = $sb;
										}
	...
	public function pdf_profit_loss()
	{
		...
		if ($breakdown == "monthly") {
			$paper_size = 'A2';
			if(count($getMonthsInRange) < 3) $paper_size = 'A4';
			if(count($getMonthsInRange) > 3 && count($getMonthsInRange) < 8) $paper_size = 'A3';
	...
	public function pdf_trial_balance_new()
	{
		...
		$options->set(array('isRemoteEnabled' => true));
		$dompdf->setOptions($options);
		$dompdf->loadHtml(view('account_report/tb_pdf', $data));
