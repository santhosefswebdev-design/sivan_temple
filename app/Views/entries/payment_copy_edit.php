    ...
    <div class="form-group">
        ...
        <input type="date" class="form-control" name="payment_date" id="payment_date" 
               value="<?php echo date("Y-m-d"); ?>"
												autocomplete="off" <?php echo htmlspecialchars($min_date . $max_date . $disabled); ?>>
										</div>
									</div>
													...
													<table class="table table-bordered cart-table" style="overflow-y: scroll;height:150px;">
														...
														<?php
															...
															<tr class="all_close" data-id="<?php echo $entrie_item['ledger_id']; ?>" id="remov<?php echo $j; ?>">
																<td><input type="hidden" name="entries[<?php echo $j; ?>][entryitemid]" value="<?php echo $entrie_item['id']; ?>"><a class="btn btn-info" style="font-size: 15px;cursor: pointer;color: #fff;font-weight: bold;padding: 0px 5px;" onclick="remove(<?php echo $j; ?>)" id="remove">X</a></td>
																<td><input type="hidden" style="text-align: center;" id="edit_ledgerid_<?php echo $j; ?>" class="row_amt" name="entries[<?php echo $j; ?>][ledgerid]" value="<?php echo $entrie_item['ledger_id']; ?>">
																<select class="form-control search_box" data-live-search="true" data-live-search-style="startsWith" id="payment_credit_ac_new_<?php echo $j; ?>" onchange="edit_reselectledger(<?php echo $j; ?>,this.value)">
																	<option value="">Select debit a/c</option>
																	<?php
																...
																<td><input type="text" style="text-align: center;" class="row_tot" name="entries[<?php echo $j; ?>][amount]" id="tot_<?php echo $j; ?>" value="<?php echo $entrie_item['amount']; ?>"></td>
...
$('#payment_amount').click(function() {
    ...
    $("#submit").click(function(){
        ...
        ({
            type:"POST",
            url: "<?php echo base_url(); ?>/entries/save_payment_copy_entries",
            data: $("form").serialize(),
...
$(function() {
   ...
   /*  $('#row_transaction_online').hide(); 
	...
	/* $('#payment_paymode').trigger('change'); */
	...
	$('#addcreditdetail').click(function(){
      ...
      //  var cnt = parseInt($("#pay_row_count").val());
        if(payment_credit_ac != '' && payment_amount != "0.00" && payment_amount != "0"){
			...
			text1 += '<td><input type="hidden" style="text-align: center;" id="ledgerid_'+i+'"  class="row_amt" name="entries['+i+'][ledgerid]" value="'+credit_ac+'"><select class="form-control search_box" data-live-search="true" data-live-search-style="startsWith" id="payment_credit_ac_new_'+i+'" onchange="reselectledger('+i+',this.value)"><option value="">Select debit a/c</option><?php
			...
			text1 += '<td><input type="number" style="text-align: center;" class="row_tot" id="tot_'+i+'" name="entries['+i+'][amount]" value="'+amt+'"></td>';
