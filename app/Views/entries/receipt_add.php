								...
								<div class="row">
									<div class="col-md-5">
										<div class="form-group">
											...
											<select class="form-control search_box" data-live-search="true" data-live-search-style="startsWith" name="receipt_debit_ac" id="receipt_debit_ac" <?php echo $disabled; ?>>
...
document.getElementById('receipt_date').addEventListener('change', function() {
    ...
    var uid = 1;  // Assuming '1' corresponds to receipts, adjust based on your logic

    if (selectedDate) {
        $.ajax({
            url: '<?php echo base_url(); ?>/entries/generateReceiptEntryCodeByDate', // Adjust URL as needed
				type: 'POST',
				data: { date: selectedDate, uid: uid },
				dataType: 'json',
				success: function (response) {
					if (response.entryCode) {
						$('#receipt_entrycode').val(response.entryCode);
					} else {
